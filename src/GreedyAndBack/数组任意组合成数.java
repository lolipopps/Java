 ...
 */
	public static void main(String[] args) {
	 ...
	 public static List<List<Integer>> combinationSum1(int[] candidates, int target) 
	    ...
	    {
	        if(remain < 0) // 如果没有数字减完了。退出
	            return false; 
	        else if(remain == 0)  // 恰好等于 0 证明正好是个解 退出
	        {
	        ...
	        {
	            for(int i=start; i<nums.length; i++) // 从第其实位置开始
	            {
	                boolean flag;
	                tempList.add(nums[i]);
	                flag = backtrack(list, tempList, nums, remain - nums[i], i); //可以用相同的数字可以每次都从 1开始.
