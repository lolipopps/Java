...
public class ReadExcelUtils {
	...
	public static ArrayList<ArrayList<String>> readExcel2003(HSSFWorkbook workbook, String fileName)
			throws IOException {
		ArrayList<ArrayList<String>> contentList = new ArrayList<ArrayList<String>>();
		...
		for (int numSheets = 0; numSheets < workbook.getNumberOfSheets(); numSheets++) {
			...
			if (null != workbook.getSheetAt(numSheets)) {
				...
				// System.out.println(contentModle.getSheetName() + "总行数：" +
				// aSheet.getLastRowNum() + 1);
				for (int rowNumOfSheet = 0; rowNumOfSheet <= aSheet.getLastRowNum(); rowNumOfSheet++) {
					...
					if (null != aSheet.getRow(rowNumOfSheet)) {
						...
						for (int cellNumOfRow = 0; cellNumOfRow < aRow.getLastCellNum(); cellNumOfRow++) {
							...
							if (!pattern.matcher(contentModle.getContent()).matches()) {
								break;
							} else {
								// 单元格
								if (contentModle.getContent().length() < 7) {
									row.add(fileName.split(".")[0]);
								} else {
									row.add(fileName.split(".")[0]);
							...
							// System.out.print("(" + contentModle.getRowNum() + "," +
							// contentModle.getCellNumOfRow() + ")" + contentModle.getContent());
							// System.out.print(" ");
						...
						// 添加一行
						if (aRow.getFirstCellNum() < 6) {
							for (int i = aRow.getFirstCellNum(); i <= 6; i++) {
								row.add("");
							}
						}
						contentList.add(row);
	...
	public static ArrayList<ArrayList<String>> readExcel2007(XSSFWorkbook workbook, String fileName)
			throws IOException {
		ArrayList<ArrayList<String>> contentList = new ArrayList<ArrayList<String>>();
		// StringBuilder content = new StringBuilder();
		// // 创建对Excel工作簿文件的引用
